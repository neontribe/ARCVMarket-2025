...
const NetMgrFactory = function (config) {
  return {
    ...
    isAuth: function () {
      if (this.token) {
        const expiryTime = this.token.requestTime + this.token.expres_in;
  ...
  if (origResponse.status !== 202) {
    ...
    switch (status) {
      ...
      case 'finished':
        ...
        return await pollMgr.axiosInstance.get(pollingResponse.data.location)
      default:
  ...
  if (origResp.status === 401 && !origCfg._retry && NetMgr.token) {
    switch (origResp.data.error) {
      ...
      case 'Unauthenticated.': // User not logged on.
        ...
        const lsToken = NetMgr.getTokenFromLocalStorage() || NetMgr.token
