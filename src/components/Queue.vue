<template id="queue">
  <transition v-if="currentlyShown" name="fade">
    <div class="content narrow queuedVouchers">
      ...
      <div class="list-wrapper" v-bind:class="{ 'is-collapsed': collapsed }">
        <div class="voucher-list">
          ...
          <!-- Tab row -->
          <div v-for="voucher in vouchers" :key="voucher.code" class="tab row">
            <label>
              <div class="row-code">
                <div>{{ voucher.code }}</div>
              </div>
...
export default {
  ...
  computed: {
    currentlyShown: function () {
      return this.vouchers.length >= 1 && !Store.getVouchersOnlineStatus();
    },
